    ...
    <article onClick={handleCardClick} className="post">
      <header className="post__header">
        <img
          ...
          src={post.image?.[0] || 'https://concepto.de/wp-content/uploads/2015/03/paisaje-e1549600034372.jpg'} alt="" 
        />
      </header>
      ...
      <section className="post__footer">
        <header className="post__avatar">
          <img className="post__avatar-img" src={post.user.profileImgUrl} alt="" />
        </header>
        <article className="post__footer-info">
          <h4 className="post__nameUser">{post.user.name}</h4>
          <p className="post__date">{post.updatedAt}</p>
        </article>
